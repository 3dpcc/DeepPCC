...
class Pct_2_0(nn.Module):
    def __init__(self, input_channel):
        super(Pct_2_0, self).__init__()
    ...
    def forward(self, x):
        ...
        xyz = x_C.squeeze(0)[:, 1:]
        new_xyz = index_points(x_C[:, :, 1:], idx).squeeze(0)
        new_xyz = xyz[:, None, :] - new_xyz[:, :, :]
        xyz_enc = self.pos(new_xyz)

        new_feature = index_points(x_F, idx).squeeze(0)
        # knn
        ...
        new_feature = index_points(out_F, idx).squeeze(0)
