...
class conv_out_u(torch.nn.Module):
...
class Module_u(torch.nn.Module):
    def __init__(self, channels=128):
        super().__init__()
        # pre processing
        ...
        self.conv_out = conv_out_u(channels=128)
...
class Mutiscale_enhancer_U(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.conv_in_y = ME.MinkowskiConvolution(
            ...
            bias=True,
            dimension=3)
        self.conv_in_uv = ME.MinkowskiConvolution(
            in_channels=1,
            out_channels=16,
            kernel_size=3,
            stride=1,
            bias=True,
            dimension=3)
        self.model_U = Module_u()
        self.relu = ME.MinkowskiReLU(inplace=True)

    def forward(self, y, u):
        input_y = self.relu(self.conv_in_y(y))
        input_u = self.relu(self.conv_in_uv(u))

        feats = torch.cat((input_y.F, input_u.F), dim=1)
        ...
        out = self.model_U(yuv)

        return out

class Enhancer_uv(torch.nn.Module):
    def __init__(self, channels=128, A=3):
        super(Enhancer_uv, self).__init__()
        self.conv_in_y = ME.MinkowskiConvolution(in_channels=1,
            out_channels=16,
            kernel_size=3,
            stride=1,
            bias=True,
            dimension=3)
        self.conv_in_uv = ME.MinkowskiConvolution(in_channels=2,
            out_channels=16,
            kernel_size=3,
            stride=1,
            bias=True,
    ...
    def forward(self, y, uv):
        out_y = self.relu(self.conv_in_y(y))
        out_uv = self.relu(self.conv_in_uv(uv))
        out = ME.SparseTensor(features=torch.cat((out_y.F, out_uv.F), dim=1), coordinate_manager=out_uv.coordinate_manager,
                              coordinate_map_key=out_uv.coordinate_map_key)
